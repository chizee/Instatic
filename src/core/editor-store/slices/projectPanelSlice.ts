 ...
 */
export type PackageJson = ProjectPackageJson

export interface ProjectPanelSlice {
  /**
   * In-memory package.json manifest.
   * Tracks intended project deps; installing is a separate bridge concern.
   */
...
export const createProjectPanelSlice: StateCreator<
  EditorStore,
  [],
  [],
  ProjectPanelSlice
  ...
  setDependency: (name, version, dev = false) => {
    ...
    set((s) => {
      ...
      return {
        ...
        project: s.project
          ? { ...s.project, packageJson: nextPackageJson, updatedAt: Date.now() }
          : s.project,
      }
    })
  },
