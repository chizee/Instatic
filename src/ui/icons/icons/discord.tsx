import React from 'react';
import type { IconProps } from '../types';

export function DiscordIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="3" y="5" width="2" height="2" fill={color}/>
      <rect x="3" y="17" width="2" height="2" fill={color}/>
      <rect x="19" y="5" width="2" height="2" fill={color}/>
      <rect x="19" y="17" width="2" height="2" fill={color}/>
      <rect x="1" y="7" width="2" height="10" fill={color}/>
      <rect x="21" y="7" width="2" height="10" fill={color}/>
      <rect x="9" y="17" width="2" height="2" fill={color}/>
      <rect x="13" y="17" width="2" height="2" fill={color}/>
      <rect x="5" y="19" width="4" height="2" fill={color}/>
      <rect x="15" y="19" width="4" height="2" fill={color}/>
      <rect x="7" y="15" width="10" height="2" fill={color}/>
      <rect x="5" y="3" width="4" height="2" fill={color}/>
      <rect x="15" y="3" width="4" height="2" fill={color}/>
      <rect x="9" y="5" width="6" height="2" fill={color}/>
      <rect x="13" y="10" width="3" height="3" fill={color}/>
      <rect x="8" y="10" width="3" height="3" fill={color}/>
    </svg>
  );
}
