import React from 'react';
import type { IconProps } from '../types';

export function AwsIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="3" y="17" width="4" height="2" fill={color}/>
      <rect x="7" y="19" width="8" height="2" fill={color}/>
      <rect x="15" y="17" width="2" height="2" fill={color}/>
      <rect x="15" y="13" width="6" height="2" fill={color}/>
      <rect x="19" y="13" width="2" height="6" fill={color}/>
      <rect x="2" y="5" width="2" height="6" fill={color}/>
      <rect x="8" y="3" width="2" height="8" fill={color}/>
      <rect x="5" y="5" width="2" height="6" fill={color}/>
      <rect x="13" y="3" width="2" height="8" fill={color}/>
      <rect x="2" y="4" width="5" height="1" fill={color}/>
      <rect x="3" y="3" width="3" height="1" fill={color}/>
      <rect x="17" y="3" width="3" height="2" fill={color}/>
      <rect x="17" y="6" width="3" height="2" fill={color}/>
      <rect x="17" y="9" width="3" height="2" fill={color}/>
      <rect x="2" y="6" width="5" height="2" fill={color}/>
      <rect x="16" y="5" width="2" height="1" fill={color}/>
      <rect x="19" y="8" width="2" height="1" fill={color}/>
      <rect x="10" y="8" width="1" height="2" fill={color}/>
      <rect x="11" y="7" width="1" height="2" fill={color}/>
      <rect x="12" y="8" width="1" height="2" fill={color}/>
      <rect x="16" y="4" width="5" height="1" fill={color}/>
      <rect x="16" y="9" width="5" height="1" fill={color}/>
      <rect x="20" y="7" width="1" height="1" fill={color}/>
      <rect x="16" y="6" width="1" height="1" fill={color}/>
    </svg>
  );
}
