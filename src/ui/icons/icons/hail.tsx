import React from 'react';
import type { IconProps } from '../types';

export function HailIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="4" y="12" width="6" height="2" fill={color}/>
      <rect x="16" y="15" width="3" height="2" fill={color}/>
      <rect x="4" y="3" width="3" height="2" fill={color}/>
      <rect x="2" y="14" width="2" height="6" fill={color}/>
      <rect x="14" y="17" width="2" height="3" fill={color}/>
      <rect x="2" y="5" width="2" height="3" fill={color}/>
      <rect x="4" y="20" width="6" height="2" fill={color}/>
      <rect x="16" y="20" width="3" height="2" fill={color}/>
      <rect x="4" y="8" width="3" height="2" fill={color}/>
      <rect x="10" y="14" width="2" height="6" fill={color}/>
      <rect x="19" y="17" width="2" height="3" fill={color}/>
      <rect x="7" y="5" width="2" height="3" fill={color}/>
      <rect x="14" y="11" width="2" height="2" fill={color}/>
      <rect x="16" y="9" width="2" height="2" fill={color}/>
      <rect x="11" y="6" width="2" height="2" fill={color}/>
      <rect x="18" y="7" width="2" height="2" fill={color}/>
      <rect x="13" y="4" width="2" height="2" fill={color}/>
      <rect x="20" y="5" width="2" height="2" fill={color}/>
      <rect x="15" y="2" width="2" height="2" fill={color}/>
    </svg>
  );
}
