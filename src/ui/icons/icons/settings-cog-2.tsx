import React from 'react';
import type { IconProps } from '../types';

export function SettingsCog2Icon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="18" y="2" width="2" height="2" fill={color}/>
      <rect x="4" y="2" width="2" height="2" fill={color}/>
      <rect x="20" y="22" width="2" height="2" transform="rotate(180 20 22)" fill={color}/>
      <rect width="2" height="2" transform="matrix(1 0 0 -1 4 22)" fill={color}/>
      <rect x="20" y="4" width="2" height="2" fill={color}/>
      <rect x="6" y="4" width="4" height="2" fill={color}/>
      <rect x="18" y="20" width="4" height="2" transform="rotate(180 18 20)" fill={color}/>
      <rect width="4" height="2" transform="matrix(1 0 0 -1 6 20)" fill={color}/>
      <rect x="18" y="6" width="2" height="4" fill={color}/>
      <rect x="4" y="6" width="2" height="4" fill={color}/>
      <rect x="20" y="18" width="2" height="4" transform="rotate(180 20 18)" fill={color}/>
      <rect width="2" height="4" transform="matrix(1 0 0 -1 4 18)" fill={color}/>
      <rect x="14" y="4" width="4" height="2" fill={color}/>
      <rect x="2" y="4" width="2" height="2" fill={color}/>
      <rect x="22" y="20" width="2" height="2" transform="rotate(180 22 20)" fill={color}/>
      <rect width="2" height="2" transform="matrix(1 0 0 -1 2 20)" fill={color}/>
      <rect x="8" y="2" width="2" height="4" fill={color}/>
      <rect width="2" height="4" transform="matrix(1 0 0 -1 8 22)" fill={color}/>
      <rect x="8" y="2" width="8" height="2" fill={color}/>
      <rect width="8" height="2" transform="matrix(1 0 0 -1 8 22)" fill={color}/>
      <rect x="2" y="8" width="2" height="8" fill={color}/>
      <rect x="22" y="16" width="2" height="8" transform="rotate(180 22 16)" fill={color}/>
      <rect x="20" y="8" width="2" height="4" fill={color}/>
      <rect x="10" y="8" width="4" height="2" fill={color}/>
      <rect x="8" y="10" width="2" height="4" fill={color}/>
      <rect x="10" y="14" width="4" height="2" fill={color}/>
      <rect x="14" y="10" width="2" height="4" fill={color}/>
    </svg>
  );
}
