import React from 'react';
import type { IconProps } from '../types';

export function MicrosoftOutlookIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="3" y="10" width="2" height="4" fill={color}/>
      <rect x="4" y="9" width="4" height="2" fill={color}/>
      <rect x="4" y="13" width="4" height="2" fill={color}/>
      <rect x="7" y="10" width="2" height="4" fill={color}/>
      <rect x="2" y="6" width="8" height="2" fill={color}/>
      <rect y="8" width="2" height="8" fill={color}/>
      <rect x="2" y="16" width="8" height="2" fill={color}/>
      <rect x="10" y="8" width="2" height="8" fill={color}/>
      <rect x="20" y="12" width="2" height="8" fill={color}/>
      <rect x="5" y="4" width="2" height="2" fill={color}/>
      <rect x="5" y="18" width="2" height="2" fill={color}/>
      <rect x="7" y="20" width="13" height="2" fill={color}/>
      <rect x="16" y="18" width="4" height="2" fill={color}/>
      <rect x="12" y="16" width="4" height="2" fill={color}/>
      <rect x="10" y="14" width="2" height="2" fill={color}/>
      <rect x="16" y="14" width="2" height="2" fill={color}/>
      <rect x="18" y="12" width="2" height="2" fill={color}/>
      <rect x="7" y="2" width="11" height="2" fill={color}/>
      <rect x="18" y="4" width="2" height="8" fill={color}/>
      <rect x="10" y="6" width="6" height="2" fill={color}/>
      <rect x="10" y="10" width="6" height="2" fill={color}/>
    </svg>
  );
}
