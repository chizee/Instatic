import React from 'react';
import type { IconProps } from '../types';

export function WordpressIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      {/* Pixelated circle outline */}
      <rect x="9" y="2" width="6" height="2" fill={color}/>
      <rect x="5" y="4" width="4" height="2" fill={color}/>
      <rect x="15" y="4" width="4" height="2" fill={color}/>
      <rect x="3" y="6" width="2" height="3" fill={color}/>
      <rect x="19" y="6" width="2" height="3" fill={color}/>
      <rect x="2" y="9" width="2" height="6" fill={color}/>
      <rect x="20" y="9" width="2" height="6" fill={color}/>
      <rect x="3" y="15" width="2" height="3" fill={color}/>
      <rect x="19" y="15" width="2" height="3" fill={color}/>
      <rect x="5" y="18" width="4" height="2" fill={color}/>
      <rect x="15" y="18" width="4" height="2" fill={color}/>
      <rect x="9" y="20" width="6" height="2" fill={color}/>

      {/* W mark inside */}
      <rect x="6" y="9" width="2" height="5" fill={color}/>
      <rect x="8" y="14" width="2" height="2" fill={color}/>
      <rect x="10" y="12" width="2" height="3" fill={color}/>
      <rect x="12" y="12" width="2" height="3" fill={color}/>
      <rect x="14" y="14" width="2" height="2" fill={color}/>
      <rect x="16" y="9" width="2" height="5" fill={color}/>
    </svg>
  );
}
