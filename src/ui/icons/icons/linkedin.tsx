import React from 'react';
import type { IconProps } from '../types';

export function LinkedinIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="1" y="8" width="6" height="2" fill={color}/>
      <rect x="1" y="20" width="6" height="2" fill={color}/>
      <rect x="1" y="10" width="2" height="10" fill={color}/>
      <rect x="5" y="10" width="2" height="10" fill={color}/>
      <rect x="9" y="20" width="6" height="2" fill={color}/>
      <rect x="9" y="12" width="2" height="8" fill={color}/>
      <rect x="9" y="10" width="2" height="10" fill={color}/>
      <rect x="13" y="8" width="6" height="2" fill={color}/>
      <rect x="17" y="20" width="6" height="2" fill={color}/>
      <rect x="21" y="12" width="2" height="8" fill={color}/>
      <rect x="17" y="14" width="2" height="6" fill={color}/>
      <rect x="13" y="12" width="4" height="2" fill={color}/>
      <rect x="9" y="8" width="4" height="2" fill={color}/>
      <rect x="19" y="10" width="2" height="2" fill={color}/>
      <rect x="13" y="14" width="2" height="6" fill={color}/>
      <rect x="3" y="1" width="2" height="2" fill={color}/>
      <rect x="1" y="3" width="2" height="2" fill={color}/>
      <rect x="3" y="5" width="2" height="2" fill={color}/>
      <rect x="5" y="3" width="2" height="2" fill={color}/>
    </svg>
  );
}
