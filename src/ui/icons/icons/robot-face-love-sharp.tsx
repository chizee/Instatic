import React from 'react';
import type { IconProps } from '../types';

export function RobotFaceLoveSharpIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <g clip-path="url(#clip0_496_1501)">
      <rect x="2" y="4" width="18" height="2" fill={color}/>
      <rect x="2" y="18" width="10" height="2" fill={color}/>
      <rect x="2" y="6" width="2" height="12" fill={color}/>
      <rect x="18" y="6" width="2" height="6" fill={color}/>
      <rect x="11" width="2" height="6" fill={color}/>
      <rect x="6" y="12" width="2" height="2" fill={color}/>
      <rect x="12" y="12" width="2" height="2" fill={color}/>
      <rect x="8" y="14" width="4" height="2" fill={color}/>
      <rect x="6" y="8" width="2" height="2" fill={color}/>
      <rect x="12" y="8" width="2" height="2" fill={color}/>
      <rect x="13" width="4" height="2" fill={color}/>
      <rect y="10" width="2" height="2" fill={color}/>
      <rect x="18" y="10" width="4" height="2" fill={color}/>
      <rect x="16" y="14" width="2" height="2" fill={color}/>
      <rect x="18" y="16" width="2" height="2" fill={color}/>
      <rect x="14" y="16" width="2" height="4" fill={color}/>
      <rect x="16" y="20" width="2" height="2" fill={color}/>
      <rect x="18" y="22" width="2" height="2" fill={color}/>
      <rect x="20" y="20" width="2" height="2" fill={color}/>
      <rect x="22" y="16" width="2" height="4" fill={color}/>
      <rect x="20" y="14" width="2" height="2" fill={color}/>
      </g>
      <defs>
      <clipPath id="clip0_496_1501">
      <rect width="24" height="24" fill="white"/>
      </clipPath>
      </defs>
    </svg>
  );
}
