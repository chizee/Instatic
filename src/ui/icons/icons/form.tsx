import React from 'react';
import type { IconProps } from '../types';

export function FormIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="3" y="2" width="12" height="2" fill={color}/>
      <rect x="3" y="13" width="8" height="2" fill={color}/>
      <rect x="5" y="5" width="14" height="2" fill={color}/>
      <rect x="5" y="16" width="14" height="2" fill={color}/>
      <rect x="5" y="9" width="14" height="2" fill={color}/>
      <rect x="5" y="20" width="14" height="2" fill={color}/>
      <rect x="3" y="7" width="2" height="2" fill={color}/>
      <rect x="3" y="18" width="2" height="2" fill={color}/>
      <rect x="19" y="7" width="2" height="2" fill={color}/>
      <rect x="19" y="18" width="2" height="2" fill={color}/>
    </svg>
  );
}
