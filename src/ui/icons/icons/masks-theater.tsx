import React from 'react';
import type { IconProps } from '../types';

export function MasksTheaterIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <g clipPath="url(#clip0_496_1167)">
        <rect x="14" y="18" width="4" height="2" fill={color}/>
        <rect width="4" height="2" transform="matrix(1 0 0 -1 6 10)" fill={color}/>
        <rect x="12" y="16" width="2" height="2" fill={color}/>
        <rect width="2" height="2" transform="matrix(1 0 0 -1 4 12)" fill={color}/>
        <rect x="18" y="16" width="2" height="2" fill={color}/>
        <rect x="12" y="12" width="2" height="2" fill={color}/>
        <rect x="4" y="4" width="2" height="2" fill={color}/>
        <rect x="18" y="12" width="2" height="2" fill={color}/>
        <rect x="10" y="4" width="2" height="2" fill={color}/>
        <rect x="12" y="8" width="10" height="2" fill={color}/>
        <rect x="2" width="12" height="2" fill={color}/>
        <rect x="8" y="12" width="2" height="10" fill={color}/>
        <rect y="2" width="2" height="12" fill={color}/>
        <rect x="22" y="10" width="2" height="12" fill={color}/>
        <rect x="14" y="2" width="2" height="4" fill={color}/>
        <rect x="10" y="22" width="12" height="2" fill={color}/>
        <rect x="2" y="14" width="4" height="2" fill={color}/>
      </g>
      <defs>
        <clipPath id="clip0_496_1167">
          <rect width="24" height="24" fill="white"/>
        </clipPath>
      </defs>
    </svg>
  );
}
