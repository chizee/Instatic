import React from 'react';
import type { IconProps } from '../types';

export function PixelarticonsIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="7" y="13" width="2" height="2" fill={color}/>
      <rect x="7" y="7" width="6" height="2" fill={color}/>
      <rect x="4" y="2" width="16" height="2" fill={color}/>
      <rect x="4" y="20" width="16" height="2" fill={color}/>
      <rect x="2" y="20" width="16" height="2" transform="rotate(-90 2 20)" fill={color}/>
      <rect x="20" y="20" width="16" height="2" transform="rotate(-90 20 20)" fill={color}/>
      <rect x="7" y="11" width="6" height="2" fill={color}/>
      <rect x="11" y="9" width="2" height="2" fill={color}/>
      <rect x="7" y="9" width="2" height="2" fill={color}/>
      <rect x="13" y="13" width="2" height="2" fill={color}/>
      <rect x="15" y="11" width="2" height="2" fill={color}/>
      <rect x="15" y="15" width="2" height="2" fill={color}/>
      <rect x="11" y="15" width="2" height="2" fill={color}/>
    </svg>
  );
}
