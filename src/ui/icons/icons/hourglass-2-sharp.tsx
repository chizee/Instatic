import React from 'react';
import type { IconProps } from '../types';

export function Hourglass2SharpIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} style={style}>
      <rect x="6" y="2" width="12" height="2" fill={color}/>
      <rect x="6" y="4" width="2" height="4" fill={color}/>
      <rect x="16" y="4" width="2" height="4" fill={color}/>
      <rect x="14" y="8" width="2" height="2" fill={color}/>
      <rect x="8" y="8" width="2" height="2" fill={color}/>
      <rect x="10" y="8" width="4" height="4" fill={color}/>
      <rect width="12" height="4" transform="matrix(1 0 0 -1 6 22)" fill={color}/>
      <rect width="2" height="4" transform="matrix(1 0 0 -1 6 20)" fill={color}/>
      <rect width="2" height="4" transform="matrix(1 0 0 -1 16 20)" fill={color}/>
      <rect width="2" height="2" transform="matrix(1 0 0 -1 14 16)" fill={color}/>
      <rect width="2" height="2" transform="matrix(1 0 0 -1 8 16)" fill={color}/>
      <rect width="4" height="2" transform="matrix(1 0 0 -1 10 14)" fill={color}/>
      <rect x="11" y="14" width="2" height="4" fill={color}/>
    </svg>
  );
}
