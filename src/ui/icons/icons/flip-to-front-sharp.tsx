import React from 'react';
import type { IconProps } from '../types';

export function FlipToFrontSharpIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="7" y="3" width="2" height="14" fill={color}/>
      <rect x="19" y="3" width="2" height="14" fill={color}/>
      <rect x="9" y="3" width="10" height="2" fill={color}/>
      <rect x="9" y="15" width="10" height="2" fill={color}/>
      <rect x="3" y="4" width="2" height="2" fill={color}/>
      <rect x="3" y="9" width="2" height="2" fill={color}/>
      <rect x="3" y="14" width="2" height="2" fill={color}/>
      <rect x="3" y="19" width="2" height="2" fill={color}/>
      <rect x="8" y="19" width="2" height="2" fill={color}/>
      <rect x="13" y="19" width="2" height="2" fill={color}/>
      <rect x="18" y="19" width="2" height="2" fill={color}/>
    </svg>
  );
}
