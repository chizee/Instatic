import React from 'react';
import type { IconProps } from '../types';

export function PnpmIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="2" y="2" width="6" height="2" fill={color}/>
      <rect x="9" y="2" width="6" height="2" fill={color}/>
      <rect x="16" y="2" width="6" height="2" fill={color}/>
      <rect x="2" y="6" width="6" height="2" fill={color}/>
      <rect x="9" y="6" width="6" height="2" fill={color}/>
      <rect x="16" y="6" width="6" height="2" fill={color}/>
      <rect x="2" y="4" width="2" height="2" fill={color}/>
      <rect x="9" y="4" width="2" height="2" fill={color}/>
      <rect x="16" y="4" width="2" height="2" fill={color}/>
      <rect x="6" y="4" width="2" height="2" fill={color}/>
      <rect x="13" y="4" width="2" height="2" fill={color}/>
      <rect x="20" y="4" width="2" height="2" fill={color}/>
      <rect x="16" y="9" width="6" height="2" fill={color}/>
      <rect x="16" y="13" width="6" height="2" fill={color}/>
      <rect x="16" y="11" width="2" height="2" fill={color}/>
      <rect x="20" y="11" width="2" height="2" fill={color}/>
      <rect x="16" y="16" width="6" height="2" fill={color}/>
      <rect x="16" y="20" width="6" height="2" fill={color}/>
      <rect x="16" y="18" width="2" height="2" fill={color}/>
      <rect x="20" y="18" width="2" height="2" fill={color}/>
      <rect x="9" y="16" width="6" height="2" fill={color}/>
      <rect x="9" y="20" width="6" height="2" fill={color}/>
      <rect x="9" y="18" width="2" height="2" fill={color}/>
      <rect x="13" y="18" width="2" height="2" fill={color}/>
      <rect x="2" y="16" width="6" height="2" fill={color}/>
      <rect x="2" y="20" width="6" height="2" fill={color}/>
      <rect x="2" y="18" width="2" height="2" fill={color}/>
      <rect x="6" y="18" width="2" height="2" fill={color}/>
      <rect x="9" y="9" width="6" height="2" fill={color}/>
      <rect x="9" y="13" width="6" height="2" fill={color}/>
      <rect x="9" y="11" width="2" height="2" fill={color}/>
      <rect x="13" y="11" width="2" height="2" fill={color}/>
    </svg>
  );
}
