import React from 'react';
import type { IconProps } from '../types';

export function Gamepad2SharpIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="7" y="8" width="2" height="6" fill={color}/>
      <rect x="5" y="10" width="6" height="2" fill={color}/>
      <rect x="14" y="11" width="2" height="2" fill={color}/>
      <rect x="17" y="9" width="2" height="2" fill={color}/>
      <rect x="3" y="4" width="18" height="2" fill={color}/>
      <rect x="1" y="4" width="2" height="16" fill={color}/>
      <rect x="21" y="4" width="2" height="16" fill={color}/>
      <rect x="3" y="18" width="6" height="2" fill={color}/>
      <rect x="15" y="18" width="6" height="2" fill={color}/>
      <rect x="7" y="16" width="10" height="2" fill={color}/>
    </svg>
  );
}
