import React from 'react';
import type { IconProps } from '../types';

export function CatIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="4" y="2" width="4" height="2" fill={color}/>
      <rect width="4" height="2" transform="matrix(-1 0 0 1 20 2)" fill={color}/>
      <rect x="2" y="2" width="2" height="6" fill={color}/>
      <rect width="2" height="6" transform="matrix(-1 0 0 1 22 2)" fill={color}/>
      <rect x="4" y="8" width="2" height="2" fill={color}/>
      <rect width="2" height="2" transform="matrix(-1 0 0 1 20 8)" fill={color}/>
      <rect x="2" y="10" width="2" height="8" fill={color}/>
      <rect width="2" height="8" transform="matrix(-1 0 0 1 22 10)" fill={color}/>
      <rect x="4" y="18" width="2" height="2" fill={color}/>
      <rect width="2" height="2" transform="matrix(-1 0 0 1 20 18)" fill={color}/>
      <rect x="6" y="20" width="12" height="2" fill={color}/>
      <rect x="8" y="4" width="8" height="2" fill={color}/>
      <rect x="6" y="12" width="2" height="2" fill={color}/>
      <rect x="16" y="12" width="2" height="2" fill={color}/>
      <rect x="9" y="14" width="6" height="2" fill={color}/>
      <rect x="11" y="16" width="2" height="2" fill={color}/>
    </svg>
  );
}
