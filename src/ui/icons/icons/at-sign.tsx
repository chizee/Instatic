import React from 'react';
import type { IconProps } from '../types';

export function AtSignIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="9" y="8" width="6" height="2" fill={color}/>
      <rect x="9" y="14" width="8" height="2" fill={color}/>
      <rect x="7" y="14" width="4" height="2" transform="rotate(-90 7 14)" fill={color}/>
      <rect x="13" y="14" width="6" height="2" transform="rotate(-90 13 14)" fill={color}/>
      <rect x="17" y="6" width="2" height="8" fill={color}/>
      <rect x="7" y="4" width="10" height="2" fill={color}/>
      <rect x="3" y="8" width="2" height="8" fill={color}/>
      <rect x="7" y="18" width="12" height="2" fill={color}/>
      <rect x="5" y="16" width="2" height="2" fill={color}/>
      <rect x="19" y="16" width="2" height="2" fill={color}/>
      <rect x="5" y="6" width="2" height="2" fill={color}/>
    </svg>
  );
}
