import React from 'react';
import type { IconProps } from '../types';

export function AiScanSharpIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="9" y="11" width="2" height="2" fill={color}/>
      <rect x="13" y="11" width="2" height="2" fill={color}/>
      <rect x="5" y="7" width="14" height="2" fill={color}/>
      <rect x="5" y="9" width="2" height="6" fill={color}/>
      <rect x="5" y="15" width="14" height="2" fill={color}/>
      <rect x="17" y="9" width="2" height="6" fill={color}/>
      <rect x="11" y="5" width="2" height="2" fill={color}/>
      <rect x="2" y="2" width="6" height="2" fill={color}/>
      <rect x="2" y="20" width="6" height="2" fill={color}/>
      <rect x="16" y="2" width="6" height="2" fill={color}/>
      <rect x="16" y="20" width="6" height="2" fill={color}/>
      <rect x="2" y="4" width="2" height="4" fill={color}/>
      <rect x="2" y="16" width="2" height="4" fill={color}/>
      <rect x="20" y="4" width="2" height="4" fill={color}/>
      <rect x="20" y="16" width="2" height="4" fill={color}/>
    </svg>
  );
}
