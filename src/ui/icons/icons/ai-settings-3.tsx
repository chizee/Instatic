import React from 'react';
import type { IconProps } from '../types';

export function AiSettings3Icon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="4" y="2" width="2" height="2" fill={color}/>
      <rect width="2" height="2" transform="matrix(1 0 0 -1 4 22)" fill={color}/>
      <rect x="6" y="4" width="4" height="2" fill={color}/>
      <rect width="4" height="2" transform="matrix(1 0 0 -1 6 20)" fill={color}/>
      <rect x="4" y="6" width="2" height="4" fill={color}/>
      <rect width="2" height="4" transform="matrix(1 0 0 -1 4 18)" fill={color}/>
      <rect x="2" y="4" width="2" height="2" fill={color}/>
      <rect width="2" height="2" transform="matrix(1 0 0 -1 2 20)" fill={color}/>
      <rect x="8" y="2" width="2" height="4" fill={color}/>
      <rect width="2" height="4" transform="matrix(1 0 0 -1 8 22)" fill={color}/>
      <rect x="2" y="8" width="2" height="8" fill={color}/>
      <rect x="10" y="8" width="3" height="2" fill={color}/>
      <rect x="14" y="11" width="4" height="2" fill={color}/>
      <rect x="16" y="3" width="2" height="2" fill={color}/>
      <rect x="14" y="5" width="2" height="2" fill={color}/>
      <rect x="16" y="19" width="2" height="2" fill={color}/>
      <rect x="14" y="17" width="2" height="2" fill={color}/>
      <rect x="10" y="2" width="3" height="2" fill={color}/>
      <rect x="10" y="20" width="3" height="2" fill={color}/>
      <rect x="8" y="10" width="2" height="4" fill={color}/>
      <rect x="10" y="14" width="3" height="2" fill={color}/>
      <rect x="18" y="9" width="2" height="2" fill={color}/>
      <rect x="18" y="1" width="2" height="2" fill={color}/>
      <rect x="18" y="17" width="2" height="2" fill={color}/>
      <rect x="20" y="11" width="2" height="2" fill={color}/>
      <rect x="20" y="3" width="2" height="2" fill={color}/>
      <rect x="20" y="19" width="2" height="2" fill={color}/>
      <rect x="18" y="13" width="2" height="2" fill={color}/>
      <rect x="18" y="5" width="2" height="2" fill={color}/>
      <rect x="18" y="21" width="2" height="2" fill={color}/>
    </svg>
  );
}
