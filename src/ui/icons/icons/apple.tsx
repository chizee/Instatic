import React from 'react';
import type { IconProps } from '../types';

export function AppleIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="11" y="8" width="3" height="2" fill={color}/>
      <rect x="14" y="6" width="4" height="2" fill={color}/>
      <rect x="18" y="8" width="2" height="3" fill={color}/>
      <rect x="16" y="11" width="2" height="4" fill={color}/>
      <rect x="18" y="15" width="2" height="5" fill={color}/>
      <rect x="14" y="20" width="4" height="2" fill={color}/>
      <rect x="11" y="18" width="3" height="2" fill={color}/>
      <rect x="7" y="20" width="4" height="2" fill={color}/>
      <rect x="5" y="18" width="2" height="2" fill={color}/>
      <rect x="3" y="10" width="2" height="8" fill={color}/>
      <rect x="5" y="8" width="2" height="2" fill={color}/>
      <rect x="7" y="6" width="4" height="2" fill={color}/>
      <rect x="12" y="3" width="2" height="2" fill={color}/>
      <rect x="14" y="1" width="2" height="2" fill={color}/>
    </svg>
  );
}
