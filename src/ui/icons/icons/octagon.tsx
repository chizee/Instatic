import React from 'react';
import type { IconProps } from '../types';

export function OctagonIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="8" y="2" width="8" height="2" fill={color}/>
      <rect x="8" y="20" width="8" height="2" fill={color}/>
      <rect x="16" y="4" width="2" height="2" fill={color}/>
      <rect width="2" height="2" transform="matrix(-1 0 0 1 8 4)" fill={color}/>
      <rect x="18" y="6" width="2" height="2" fill={color}/>
      <rect width="2" height="2" transform="matrix(-1 0 0 1 6 6)" fill={color}/>
      <rect x="18" y="16" width="2" height="2" fill={color}/>
      <rect width="2" height="2" transform="matrix(-1 0 0 1 6 16)" fill={color}/>
      <rect x="16" y="18" width="2" height="2" fill={color}/>
      <rect width="2" height="2" transform="matrix(-1 0 0 1 8 18)" fill={color}/>
      <rect x="20" y="8" width="2" height="8" fill={color}/>
      <rect width="2" height="8" transform="matrix(-1 0 0 1 4 8)" fill={color}/>
    </svg>
  );
}
