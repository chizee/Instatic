import React from 'react';
import type { IconProps } from '../types';

export function BoxStackIcon({ size = 24, color = 'currentColor', className, style }: IconProps): React.ReactElement {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      style={style}
    >
      <rect x="14" y="3" width="4" height="2" fill={color}/>
      <rect x="10" y="1" width="4" height="2" fill={color}/>
      <rect x="6" y="7" width="4" height="2" fill={color}/>
      <rect x="6" y="13" width="4" height="2" fill={color}/>
      <rect width="4" height="2" transform="matrix(-1 0 0 1 18 13)" fill={color}/>
      <rect x="6" y="19" width="4" height="2" fill={color}/>
      <rect width="4" height="2" transform="matrix(-1 0 0 1 18 19)" fill={color}/>
      <rect x="10" y="9" width="4" height="2" fill={color}/>
      <rect x="10" y="15" width="4" height="2" fill={color}/>
      <rect width="4" height="2" transform="matrix(-1 0 0 1 14 15)" fill={color}/>
      <rect x="10" y="21" width="4" height="2" fill={color}/>
      <rect width="4" height="2" transform="matrix(-1 0 0 1 14 21)" fill={color}/>
      <rect x="14" y="7" width="4" height="2" fill={color}/>
      <rect x="6" y="3" width="4" height="2" fill={color}/>
      <rect x="2" y="5" width="4" height="2" fill={color}/>
      <rect x="2" y="11" width="4" height="2" fill={color}/>
      <rect width="4" height="2" transform="matrix(-1 0 0 1 22 11)" fill={color}/>
      <rect x="2" y="17" width="4" height="2" fill={color}/>
      <rect width="4" height="2" transform="matrix(-1 0 0 1 22 17)" fill={color}/>
      <rect x="18" y="5" width="4" height="2" fill={color}/>
      <rect x="2" y="5" width="2" height="14" fill={color}/>
      <rect width="2" height="14" transform="matrix(-1 0 0 1 22 5)" fill={color}/>
      <rect x="12" y="11" width="2" height="12" fill={color}/>
    </svg>
  );
}
