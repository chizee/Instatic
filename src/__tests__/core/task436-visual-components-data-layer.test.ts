 ...
 *   src/core/page-tree/types.ts (extension)        — Project.visualComponents: VisualComponent[]
 *   src/core/persistence/validate.ts (extension)   — validateProject lenient-per-item VC handling
 *
 * ── Gate groups ──────────────────────────────────────────────────────────────
 *
 ...
 * Section 7 — validateProject extension (VP-1 – VP-7)
 *
 ...
 * @see Contribution #619 §9 — validateProject extension
...
import { validateProject } from '../../core/persistence/validate'
...
function setupProject() {
  const s = freshStore()
  s.createProject('VC Test Project')
  return useEditorStore.getState()
}
...
/** Minimal base-project fixture for validateProject tests */
function rawProject(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    id: 'proj-vc',
    name: 'VC Project',
    projectMode: 'html',
...
/** Minimal valid VC shape for validateProject tests */
...
describe('Gate TS-1 — Project.visualComponents field declared in types.ts', () => {
...
describe('Gate NV-6 — PROJECT_DUPLICATE: duplicate name in same project rejected', () => {
...
describe('Gate SL-1 — createVisualComponent: adds vc to project.visualComponents', () => {
  ...
  it('createVisualComponent adds a VC and returns its id', () => {
    ...
    const vcs = (project as unknown as { visualComponents: Array<{ id: string }> }).visualComponents
...
describe('Gate SL-2 — createVisualComponent: sets name and derives filePath', () => {
  ...
  it('created VC has correct name and derived filePath = src/components/{Name}.tsx', () => {
    ...
    const vcs = (useEditorStore.getState().project as unknown as { visualComponents: Array<{ id: string; name: string; filePath: string }> }).visualComponents
...
describe('Gate SL-9 — deleteVisualComponent: removes vc by id', () => {
  ...
  it('deleteVisualComponent removes the vc from project.visualComponents', () => {
    requireSliceAction('createVisualComponent')
    ...
    const vcs = (useEditorStore.getState().project as unknown as { visualComponents: Array<{ id: string }> }).visualComponents ?? []
...
describe('Gate SL-11 — addParam: appends a VCParam to vc.params', () => {
  ...
  it('addParam adds a param with a stable id to the VC', () => {
    ...
    const vcs = (useEditorStore.getState().project as unknown as { visualComponents: Array<{ id: string; params: Array<{ name: string; id: string }> }> }).visualComponents
...
describe('Gate SL-12 — removeParam: removes a VCParam by id', () => {
  ...
  it('removeParam removes the param from vc.params', () => {
    ...
    const vcId = (s.createVisualComponent as (name: string) => string)('Card')
    ;((useEditorStore.getState() as Record<string, unknown>).addParam as (vcId: string, name: string, type: string) => void)(vcId, 'title', 'string')
    const paramId = (useEditorStore.getState().project as unknown as { visualComponents: Array<{ id: string; params: Array<{ id: string }> }> }).visualComponents.find((v) => v.id === vcId)!.params[0].id
    ;((useEditorStore.getState() as Record<string, unknown>).removeParam as (vcId: string, paramId: string) => void)(vcId, paramId)
    const params = (useEditorStore.getState().project as unknown as { visualComponents: Array<{ id: string; params: Array<{ id: string }> }> }).visualComponents.find((v) => v.id === vcId)!.params
...
describe('Gate SL-13 — addNodeToVc: cycle guard fires at slice write boundary', () => {
  ...
  it('adding a componentRef that creates a self-cycle throws at the slice boundary', () => {
    ...
    const vcRootNodeId = (useEditorStore.getState().project as unknown as { visualComponents: Array<{ id: string; rootNode: { id: string } }> }).visualComponents.find((v) => v.id === vcId)!.rootNode.id
    expect(() =>
...
// ============================================================================
// Section 7 — validateProject extension
// ============================================================================

describe('Gate VP-1 — backward compat: missing visualComponents defaults to []', () => {
  it('project without visualComponents field validates cleanly → visualComponents: []', () => {
    const raw = rawProject()  // no visualComponents key
    delete (raw as Record<string, unknown>).visualComponents
    const project = validateProject(raw) as Project & { visualComponents?: unknown[] }
    expect(Array.isArray(project.visualComponents)).toBe(true)
    expect(project.visualComponents).toHaveLength(0)
  })
})

describe('Gate VP-2 — valid VC passes through validateProject cleanly', () => {
  it('a properly-shaped VC is preserved in the output', () => {
    const raw = rawProject({ visualComponents: [rawVC()] })
    const project = validateProject(raw) as Project & { visualComponents: Array<{ name: string }> }
    expect(project.visualComponents).toHaveLength(1)
    expect(project.visualComponents[0].name).toBe('Card')
  })
})
...
describe('Gate VP-3 — lenient: VC with invalid name is dropped', () => {
  it('a VC with a lowercase name is silently dropped (lenient per-item)', () => {
    const raw = rawProject({ visualComponents: [rawVC({ name: 'notPascal' })] })
    const project = validateProject(raw) as Project & { visualComponents: Array<unknown> }
    // Either an empty array or the bad entry is dropped — either way, no 'notPascal' vc
    const hasInvalid = project.visualComponents?.some(
...
describe('Gate VP-4 — lenient: VC with no name is dropped', () => {
  it('a VC with missing name field is silently dropped', () => {
    ...
    const raw = rawProject({ visualComponents: [badVC] })
    const project = validateProject(raw) as Project & { visualComponents: Array<unknown> }
    // Invalid entry must not survive
    const survivors = project.visualComponents ?? []
...
describe('Gate VP-5 — lenient: duplicate VC names are deduplicated (first-wins)', () => {
  it('two VCs with the same name keep only the first one', () => {
    ...
    const raw = rawProject({ visualComponents: [vc1, vc2] })
    const project = validateProject(raw) as Project & { visualComponents: Array<{ id: string }> }
    const cardVCs = project.visualComponents?.filter((v) => (v as { name: string }).name === 'Card') ?? []
...
describe('Gate VP-6 — filePath auto-corrected on mismatch', () => {
  it('a VC with mismatched filePath is corrected to src/components/{Name}.tsx', () => {
    ...
    const raw = rawProject({ visualComponents: [vc] })
    const project = validateProject(raw) as Project & { visualComponents: Array<{ name: string; filePath: string }> }
    const cardVC = project.visualComponents?.find((v) => v.name === 'Card')
    expect(cardVC?.filePath).toBe('src/components/Card.tsx')
  })
})

describe('Gate VP-7 — full project with valid VC still passes full validateProject', () => {
  it('validateProject does not throw for a project with a well-formed VC', () => {
    const raw = rawProject({ visualComponents: [rawVC()] })
...
describe('Gate VP-8 — validateProject preserves rootNode.childNodes on VC round-trip', () => {
  /**
   ...
   * tree (container → child nodes) loses all children after validateProject() is called
   ...
   */
  it('a VC rootNode with childNodes survives validateProject() with childNodes intact', () => {
    const childNode = {
      id: 'child-heading',
      moduleId: 'base.heading',
      props: { text: 'Card Title' },
    ...
    const project = validateProject(raw) as {
      visualComponents: Array<{
        rootNode: { childNodes?: unknown[] }
      }>
    }

    const resultRoot = project.visualComponents[0]?.rootNode
  ...
  it('childNodes are recursively validated (grandchildren preserved)', () => {
    ...
    const outerChild = project.visualComponents[0]?.rootNode?.childNodes?.[0]
...
describe('Gate VP-9 — validateProject preserves rootNode.propBindings on VC round-trip', () => {
  /**
   * WHY THIS GATE EXISTS
   * ─────────────────────
   * Code Reviewer (Contribution #638 / msg #1948) found that validatePageNode()
   ...
   * a VC param) loses all bindings after validateProject() on reload.
   ...
   * return shape.
   */
  it('a VC rootNode with propBindings survives validateProject() with bindings intact', () => {
  ...
  it('multiple propBindings all survive the round-trip', () => {
    ...
    const bindings = project.visualComponents[0]?.rootNode?.propBindings
  ...
  it('propBindings on childNodes also survive the round-trip', () => {
    ...
    const child = project.visualComponents[0]?.rootNode?.childNodes?.[0]
