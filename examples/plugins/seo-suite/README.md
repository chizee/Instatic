...
The sitemap uses `api.cms.content.table("pages").list({ status: "published" })` to enumerate all published pages directly. No pages need to be re-published after install for the sitemap to be complete. The `publish.html` filter captures per-page metadata (title, canonical URL, last-published timestamp) and stores it in the page index for the admin dashboard's SEO health view.
